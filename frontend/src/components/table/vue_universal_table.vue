          ...
          <table class="table table-striped" :class="{'fixedHeader' : config.fixedHeader}" style="margin: 0">
            ...
            <tbody :style="(config.fixedHeader) ? 'max-height:'+config.tbodyHeight : ''">
              <tr v-for="(singleRow, numbering) in tableData">
                ...
                >{{tableHeader.numberWithCommas ? numberWithCommas(singleRow[tableHeader.variable]) : moment(singleRow[tableHeader.variable], 'YYYY-MM-DDTHH:mm:ss.000Z', true).isValid() ? moment(singleRow[tableHeader.variable]).format('YYYY-MM-DD HH:mm:ss') : singleRow[tableHeader.variable]}}</td><!-- CONTENT HERE -->
    ...
    export default {
      ...
      methods : {
        ...
        async checkPropsPostAPI(){
          ...
          if(!this.tableHeadersFetched){
            ...
            if(this.config.fieldPropsExist){
              ...
              for(var i = 0; i < this.fields.length; i++){
                if(this.tableData.length > 0 && typeof this.tableData[0][this.fields[i].variable] == 'undefined'){//Check if Variable exist in API Response
                  this.propsValid.valid = false
                  this.propsValid.reason = `Field variable of '${this.fields[i].variable}' was not sent from the API Response`
                  return;
                }
                if(typeof this.fields[i].disableSort != 'undefined'){//If disable sort is called in object
                ...
                if(typeof this.fields[i].style_condition != 'undefined'){//If style condition is called in object
                ...
                if(typeof this.fields[i].number_formatting != 'undefined'){//If disable sort is called in object
        ...
        async fetchData(page){
          ...
          var url = `${this.api_url}${seperator}${(this.config.paginationExist) ? 'pagination=true' : ''}${this.filterUrl}`;
