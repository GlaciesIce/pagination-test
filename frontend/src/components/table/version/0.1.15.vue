<!--
  ...
  Version Changed : 2023-09-15
  
  Last Update Changes : 
  * Added path option to data and meta
  * Removed unmodifiable filter configs
  * Re-arranged the order of props and categorized them
-->
<template>
    <div class="vue_universal_table">
      ...
      <div>
        ...
        <!-- <div class="innerBox"> -->
        <div class="vue_universal_table_table" style="overflow-x: auto; overflow-y: hidden">
          ...
          <table class="table " :class="{'fixedHeader' : config.fixedHeader , 'table-striped' : config.tableStriped}" style="margin: 0">
            <!-- HEADER START -->
            <thead :style="{'background-color' : config.theadBackgroundColor}">
              <tr>
                <th class="tableTitle" v-if="config.indexing">No</th>
                <template v-for="title in tableHeaders.displayed">
                  <th
                    v-if="config.hide_column.indexOf(title.variable) === -1"
                    class="tableTitle"
            ...
            <tbody :style="(config.fixedHeader) ? 'max-height:'+config.tbodyHeight : ''">
              <tr v-for="(singleRow, numbering) in tableData" :style="processRowStyle(singleRow)">              
                <!-- Numbering/Indexing -->
                <td v-if="config.indexing" class="tableContent">{{ (config.currentPage-1)*config.limitPerPage + numbering+1 }}</td>
  
                <!-- All Data -->
                <template v-for="tableHeader in tableHeaders.displayed">
                  <td
                    v-if="config.hide_column.indexOf(tableHeader.variable) === -1"
                    class="tableContent"
                    :class="[tableHeader.class, determineCondition('class', tableHeader.class_condition, singleRow[tableHeader.variable])]"
                    :style="determineCondition('style', tableHeader.style_condition, singleRow[tableHeader.variable])"
                    @click="emit(tableHeader.variable, tableHeader.emitFull ? singleRow : singleRow[tableHeader.variable])"
                  >
                    <!-- CONTENT START -->
                    ...
                    <img
                      v-else-if="tableHeader.isImage"
                      :src="singleRow[tableHeader.variable]"
                      :title="singleRow[tableHeader.variable].split('/').pop()"
                    >
                    <span v-else>
                ...
                <td v-for="(button, index) in customButtons.title" style="text-align: center;">
                  <!-- No Icon Button -->
                  <!-- No Icon Button -->
                  <template v-if="$eval(singleRow , customButtons.condition[index])">
                    <button
  ...
  <script>
    ...
    export default {
      data () {
        return {
          ...
          config : {
            ...
            dataPath : 'resp.data.data',//Path from resp to data
            metaPath : 'resp.data.meta',//Path from resp to meta
  
            //Pagination Configs
            disableLimitPerPageFetch : false,
      ...
      props : [
        ...
        'path_to_data',//THE PATH TO DATA FROM RESP; DEFAULT 'resp.data.data' String (ex: 'resp.data')
        'path_to_meta',//THE PATH TO META FROM RESP; DEFAULT 'resp.data.meta' String (ex: 'resp.meta')
        ...
        'hide_column',//HIDE UNWANTED COLUMNS BY VARIABLE; Array of String('Column Variable Key') (ex. ['id', 'updated_at', 'deleted_at'])
        ...
        'table_striped',//Choose if table want to be Striped or not; DEFAULT true - Boolean (ex. true, false),
        'thead_background_color', // Set Background Color of Thead; DEFAULT #FFF - String Hex Code Color (ex. #D8E8ED , #D8E8EF)
        'row_style_condition',//STYLE CONDITION FOR THE WHOLE ROW; DEFAULT null - Object (ex. {'highlightRow == true' : 'background-color: yellow'})
      ...
      > 'title' [REQUIRED] = The displayed title of the field
      > 'variable' [REQUIRED] = The variable that field will take the data from
      > 'class' = CSS class that will be inserted to the 'td' element of the data
      > 'disable_sort' = disable sorting the whole field
      > 'class_condition' = If value inside the data of field matches with the ones in condition, it will show as the class in the 'td' element
      > 'style_condition' = If value inside the data of field matches with the ones in condition, it will show as the style in the 'td' element
      > 'number_formatting' = true/false, whether the data will be formatted using thousands seperator (ex. 10,000)
      > 'date_format' = Use the moment js date format input as string
      > 'is_image' = true/false, whether it is an image url that should be displayed as image or not
      > 'emit_full' = true/false, emits the full data when on${Variable} click, if not, it will just emit what's the content inside it
  
      Format of 'class_condition' and 'style_condition':
      [
        {'value' : 'class/style'},
        {'value' : 'class/style'},
        {'value' : 'class/style'}
      ...
      //Explanation:
      //'value' indicates the exact value that it must contain inorder to activate the 'class' or 'style' condition
      //'class/style', whether it is used in 'class_condition' or 'style_condition', it is the 'class' or 'style' that will be inserted into the 'td' element
  
  
  
      =============== THE CONTENT OF 'buttons': ===============
      > 'title' [REQUIRED] = The displayed title of the button and the return of the variable it is emitted from, on format of 'on${Variable}' (ex. title='lowerCase' then the emitted='onLowerCase')
      > 'variable' [OPTIONAL] = The variable that the emitted data would be, if not given, it will emit the whole row of data and must be run through 'JSON.parse()' as it has been stringified
      > 'class' = Class that will be attached to the button, all bootstrap elements such as 'btn-outline-info' and font awesome icons such as 'far fa-times' can be used here
      > 'style' = Style that will be attached to the button, making it the highest order of style to be prioritized over
      > 'condition' = An executable condition; when this statement is executed, if it returns false, the button will not be displayed. Data will be represented as 'data' object (ex. data.lat != 'N/A' || data.lng != 'N/A')
      ...
      */
      ...
      watch : {
        'update' : function () {
          if(this.config.currentlyFetching) return;
          if(this.config.infiniteScrollEnabled) this.tableData.length = 0
          this.fetchData(1);
        },
        'config.limitPerPage' : function () {
          if (!this.config.disableLimitPerPageFetch) this.fetchData(1);
          else this.config.disableLimitPerPageFetch = false;
        },
      },
      methods : {
        '$eval'(data , variableText) {
          return eval(variableText)
        },
        ...
        determineCondition(mode, condition, value){
          if(typeof condition == 'object' && condition != null){
            for(var i = 0; i < condition.length; i++){
              if(condition[i].value == value){
                return condition[i][mode]
              }
            }
          }
          return ''
        ...
        checkPropsPreAPI(){
          ...
          if(this.limit_per_page != null){
            if(this.limit_per_page_options == null || this.config.limitPerPageOptions.find(opt=>opt==this.limit_per_page)){
              if(typeof this.limit_per_page != 'number'){
                ...
                this.config.disableLimitPerPageFetch = true;
              }
            } else {
              this.propsValid.valid = false
          ...
          //CHECK table_striped
          if(this.table_striped != null){
            if(typeof this.table_striped == 'boolean'){
              this.config.tableStriped = this.table_striped;
            } else {
              this.propsValid.valid = false;
              this.propsValid.reason = "table_striped must be either a 'true' or a 'false' Boolean"
            }
          }
  
          //CHECK thead_background_color
          if(this.thead_background_color != null){
            if(typeof this.thead_background_color == 'string'){
              this.config.theadBackgroundColor = this.thead_background_color;
            } else {
              this.propsValid.valid = false;
              this.propsValid.reason = "table_striped must be a string of hex code color";
            }
          }
          ...
          if(this.pagination != null){
            if(typeof this.pagination == 'boolean'){
              ...
              if (!(this.disable_limit_per_page != null && typeof this.disable_limit_per_page == 'boolean'))
                this.config.disablePerPage = !this.pagination
            } else {
              this.propsValid.valid = false;
          ...
          if(this.buttons != null){
            if(!Array.isArray(this.buttons)){
              ...
              if(typeof this.buttons[0] == 'string'){
                ...
                var customButtonCondition = new Array(this.buttons.length).fill(true);
  
                for(var i = 0; i < this.buttons.length; i++){
  
                  //Check 'title' [REEQUIRED]
                  if(this.buttons[i].title == null){//TITLES
                    this.propsValid = false
                    this.propsValid.reason = "'buttons' props requires 'title'"
                  } else {
                    customButtonTitle[i] = this.buttons[i].title;
                  }
  
                  //Check 'variable' [REQUIRED]
                  if(this.buttons[i].variable != null && this.buttons[i].variable != ''){
                    customButtonVariable[i] = this.buttons[i].variable;
                  }
  
                  //Check 'class' and font awesome class inside it
                  ...
                  //Check 'style'
                  if(this.buttons[i].style != null){
                    customButtonStyle[i] = this.buttons[i].style;
                  }
  
                  //Check 'condition'
                  if(this.buttons[i].condition != null) {
                    customButtonCondition[i] = this.buttons[i].condition
                  }
                ...
                this.customButtons.condition = customButtonCondition;
              }
            }
          }
          ...
          //CHECK row_style_condition
          if(this.row_style_condition != null){
            if(typeof this.row_style_condition != 'object'){
              this.propsValid.valid = false
              this.propsValid.reason = `'row_style_condition' must be an object. Example: {'highlightRow == true' : 'background-color: yellow', 'highlightRow == true' : 'background-color: red'} `
            } else {
              var conditions = []
              for(var i in this.row_style_condition){
                conditions.push({
                  'condition' : i,
                  'style' : this.row_style_condition[i]
                })
              }
              this.config.rowStyleCondition = conditions;
            }
          }
  
          //CHECK hide_column
          if(this.hide_column != null){
            if(!Array.isArray(this.hide_column)){
              this.propsValid.valid = false;
              this.propsValid.reason = "hide_column must be an array of String"
            } else {
              this.config.hide_column = [];
              for(var i = 0; i < this.hide_column.length; i++){
                if(typeof this.hide_column[i] == "string") this.config.hide_column.push(this.hide_column[i])
                else {
                  this.propsValid.valid = false;
                  this.propsValid.reason = "hide_column must be an array of String"
                }
          ...
          //Check path_to_data
          if (this.path_to_data != null) {
            if (typeof this.path_to_data == 'string') this.config.dataPath = this.path_to_data
            else {
              this.propsValid.valid = false;
              this.propsValid.reason = "path_to_data must be a string"
            }
          }
  
          //Check path_to_meta
          if (this.path_to_meta != null) {
            if (typeof this.path_to_meta == 'string') this.config.metaPath = this.path_to_meta
            else {
              this.propsValid.valid = false;
              this.propsValid.reason = "path_to_meta must be a string"
            }
          }
        ...
        async checkPropsPostAPI(){
          ...
          if(!this.tableHeadersFetched){
            ...
            if(this.config.fieldPropsExist){
              ...
              var titles = [];
              var variables = [];
              var classes = new Array(this.fields.length).fill(null);
              var disableSort = new Array(this.fields.length).fill(this.config.paginationExist ? (this.disable_sort != null) ? this.disable_sort : false : true);
              var class_condition = new Array(this.fields.length).fill(null);
              ...
              var isImage = new Array(this.fields.length).fill(false);
              var emitFull = new Array(this.fields.length).fill(false);
              ...
              for(var i = 0; i < this.fields.length; i++){
                ...
                //Check 'class'
                if(typeof this.fields[i].class != 'undefined'){
                  if(typeof this.fields[i].class != 'string'){
                    this.propsValid.valid = false
                    this.propsValid.reason = `Field variable of '${this.fields[i].variable}' has an invalid 'class' request`
                    return;
                  } else {
                    classes[i] = this.fields[i].class;
                  }
                }
  
                //Check 'disable_sort'
                if(typeof this.fields[i].disable_sort != 'undefined'){
                  if(typeof this.fields[i].disable_sort != 'boolean'){
                    this.propsValid.valid = false
                    this.propsValid.reason = `Field variable of '${this.fields[i].variable}' has an invalid 'disable_sort' request`
                    return;
                  } else {
                    disableSort[i] = this.fields[i].disable_sort;
                  }
                }
  
                //Check 'class_condition'
                if(typeof this.fields[i].class_condition != 'undefined'){
                  if(typeof this.fields[i].class_condition != 'object'){
                    this.propsValid.valid = false
                    this.propsValid.reason = `Field variable of '${this.fields[i].variable}' has an invalid 'class_condition' request`
                  } else {
                    var conditions = []
                    var class_keys = Object.keys(this.fields[i].class_condition)
                    for(var j = 0; j < class_keys.length; j++){
                      conditions.push({
                        'value' : class_keys[j],
                        'class' : this.fields[i].class_condition[class_keys[j]]
                      })
                    }
                    class_condition[i] = conditions;
                  }
                }
                ...
                //Check 'is_image'
                if(typeof this.fields[i].is_image != 'undefined'){
                  if(typeof this.fields[i].is_image != 'boolean'){
                    this.propsValid.valid = false
                    this.propsValid.reason = `Field variable of '${this.fields[i].variable}' has an invalid 'is_image' request`
                    return;
                  } else {
                    isImage[i] = this.fields[i].is_image;
                  }
                }
  
                //Check 'emit_full'
                if(typeof this.fields[i].emit_full != 'undefined'){
                  if(typeof this.fields[i].emit_full != 'boolean'){
                    this.propsValid.valid = false
                    this.propsValid.reason = `Field variable of '${this.fields[i].variable}' has an invalid 'emit_full' request`
                    return;
                  } else {
                    emitFull[i] = this.fields[i].emit_full;
                  }
                }
              ...
              for(var i = 0; i < variables.length; i++){
                compiledDisplay.push({
                  ...
                  class_condition : class_condition[i],
        ...
        async fetchData(page, isExport){
          if(this.config.substituteData){//IF 'api_url' has been substituted with data
            let data = this.data;
            if(this.config.processData != null) eval(this.config.processData);
          ...
          .then((resp)=>{
            var data = eval(this.config.dataPath);
            ...
            if(isExport){
              ...
              if(vm.config.paginationExist){
                var meta = eval(this.config.metaPath);
        ...
        processRowStyle (singleRow) {
          let styles = '';
          for (var i in this.config.rowStyleCondition) {
            if (eval('singleRow.'+this.config.rowStyleCondition[i].condition)) styles += this.config.rowStyleCondition[i].style+';'
          }
          return styles;
        },
      }
  ...
  <style scoped>
    ...
    .fixedHeader tbody::-webkit-scrollbar{
      width: 0.7em;
    }
    .fixedHeader tbody::-webkit-scrollbar-track{
      background-color:#F9F9F9;
      border-radius: 20px;
    }
    .fixedHeader tbody::-webkit-scrollbar-thumb{
